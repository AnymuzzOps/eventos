...
mes_actual  = ahora.month  # número, para comparar
...
# Prompt corto — objetivo: <300 tokens por llamada
...
- Fecha: hoy ({hoy_iso}) o futura. RECHAZA si ya pasó o si es de meses anteriores a {ahora.strftime("%B")}
...
# Palabras que indican basura obvia — si el título las contiene, skip sin llamar a Groq
TITULO_BASURA = [
...
# Meses pasados en español (para pre-filtro de título)
MESES_PASADOS = {
...
def prefiltro(r: dict) -> tuple[bool, str]:
    ...
    # Menciona mes pasado SIN mencionar mes actual o futuro
...
def groq_evaluar(resultado: dict) -> dict | None:
    ...
    # Snippet truncado a 400 chars para ahorrar tokens
    ...
    try:
        ...
        if not data.get("ok"):
            print(f"  → ❌ Groq: {data.get('r','')}")
            return None

        # Post-validación fecha
